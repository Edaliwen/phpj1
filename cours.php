...
<hr>
<h3>Les variables</h3>
<p>Les variables en php sont toujours nommées avec un $. Il n'y a pas comme le js d'instanciation de variable avec un
    mot réservé comme let ou const.</p>
<?php
...
<p>Cette fonction permet des manipulations comme ucfirst(), strtoupper() et strtolower() avec plus de fonctionnalités
</p>
<p>mb_convert_case(chaine, traitement, "encodage")</p>
<ul>
    <li>chaine : la chaine de caractères à traiter</li>
    <li>traitement : une constante décrivant l'opération à effectuer (MB_CASE_UPPER, MB_CASE_LOWER, MB_CASE_TITLE). Le
        mode de traitment est en majuscules car il s'agit d'une constante, les bons usages en développement veulent que
        les constantes soient nommées en majuscules</li>
...
<ul>
    ...
    <li>
        < si la valeur est inférieure à une autre, ou <=pour inférieure ou égale</li>
...
<p>Les conditions empty() et isset() permettent de vérifier si une variable existe (isset) et si elle contient une
    valeur ou non (empty).</p>
<?php
if (isset($score2)) {
    if ($score2 == 10) {
...
<p> Le tableau à deux dimensions est un tableau qui contient un ou plusieurs tableaux en valeur, le tableau à 3
    dimensions est un tableau qui contient un ou plusieurs tableaux en valeur qui contiennent eux-mêmes d'autres
    tableuax en valeur, etc.<br>
...
<p>Cette boucle est la plus utilisée dans le cadre d'une requête mySQL pour récupérer des données (serveur) en php
    procédural uniquement.</p>
<?php
$i = 0;
$tab = [
    "lapin",
...
<p>Une constante se déclare avec la fonction define("CONSTANTE","valeur")</p>
<?php
...
<hr>
<p>S'écrivent toujours $_MOTCLEENMAJUSCULE</p>
<p>Voir la documentation pour les superGlobales prédéfinies <br>
    https://www.php.net/manual/en/language.variables.superglobals.php</p>
<?php
echo '<pre>';
var_dump($_SERVER);
echo '</pre>';
...
?>
<hr>
<h3>la superGlobale $_SESSION</h3>
<p>Elle fonctionne avec la fonction session_start() qui permet de démarrer une session et d'initialiser un tableau vide
    dans $_SESSION et avec session_destroy() qui permet de détruire la variable</p>
<?php
echo '<br>';
echo '<pre>';
var_dump($_SESSION);
echo '</pre>';
// On peut se servir de ce tableau pour déclarer un message
$_SESSION["message"] = "Vous êtes bien connecté";
?>
<hr>
<h3>la superGlobale $_GET</h3>
<p>$_GET est un tableau vide qui permet de récupérer informations sous forme de clé/valeur dans l'url avec les symboles
    ?, & et =.<br>
    Voir la documentation php au niveau de $_GET et la sécurité, les attaques xss (injection de fichier js) <br>
    Il faut utiliser la fonction htmlspecialchars()</p>
<?php
